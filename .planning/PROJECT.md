...
| **DECISION-XL-PHASER** | XL ships **Phaser** as a first-class capability for embedded mini-games, not just decorative WebGL. Three.js + Tone.js + postprocessing + physics remain auxiliary. Phaser is lazy-loaded only when active style is XL. | Brief §6/§8 list Phaser; the user upgraded it to centerpiece (mini-games, not decoration). | 🔒 Locked — Phaser implemented in Phase 6 (06-05); Tone.js + Three.js + postprocessing + physics remain in Phase 7 |
| DEC-071 | Phaser shipped in Phase 6 (06-05) — chaos scene behind XL Home, lazy-loaded via defineAsyncComponent. Three.js / Tone.js / postprocessing / physics remain Phase 7. The user clarified XL must literally feel "Unleashed" with motion behind, not just CSS gradient. | Course correction inside Phase 6: 06-04 closed the plan deck on a CSS-only XL identity; user re-affirmed XL must move. Moving Phaser forward keeps DEC-006 (lazy-loaded XL stack) intact and only changes the timing. WCAG AA is preserved by routing chaos behind a z-index: 0 canvas with a text-shadow scrim on the .home-xl-content wrapper. | 🔒 Locked — Phase 6, 2026-05-05 |
